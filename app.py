...
"""
        ...
        try:
            ...
            # 6. Display Output (THE NEW FORMAT)
            st.markdown("### ✅ Dispatch Guidance")
            
            # Clean Blue Box with References
            st.info(f"**DDG Item:** {ddg_num} \n\n **MEL Item:** {mel_num}")
            
            # Title for Text
            st.markdown("### 📖 DDG Page Text")
            
            # The Text Itself (Always Visible, No Clicking)
            st.text(raw_text)
    ...
    st.warning("Please enter a discrepancy.")import streamlit as st
import os
import json
import faiss
from sentence_transformers import SentenceTransformer
